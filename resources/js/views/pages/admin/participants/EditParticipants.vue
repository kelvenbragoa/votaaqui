...
const fetchParticipant = async () => {
    try {
        ...
        // Store original data
        originalData.value = { ...participant };
        
        // Fill form
        Object.keys(form).forEach(key => {
            if (key !== 'photo' && participant[key] !== undefined) {
                form[key] = participant[key];
            }
        });
        
        // Set photo preview if exists
        if (participant.photo_url) {
            photoPreview.value = participant.photo_url;
        }
...
const clearPhoto = () => {
    ...
    // Reset to original photo if it exists
    if (originalData.value?.photo_url) {
        photoPreview.value = originalData.value.photo_url;
    } else {
...
const submitForm = () => {
    ...
    axios.post(`/api/web/participants/${route.params.id}`, formData, {
        headers: {
    ...
    .catch((error) => {
        if (error.response?.status === 422) {
            ...
            toast.add({ 
                severity: 'error', 
                summary: 'Erro', 
                detail: error.response?.data?.message || 'Erro ao atualizar participante', 
                life: 5000 
            });
        }
    })
        ...
        <div class="w-full">
            <div class="card flex flex-col gap-4">
                <div class="flex items-center justify-between mb-4">
                    <div class="font-semibold text-xl">Editar Participante</div>
                    <Button 
