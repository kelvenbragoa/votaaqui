...
const onSubmit = handleSubmit((values) => {
    ...
    axios
        .post(`/api/paymentmethods`, values,{
            headers: {
        ...
        .then((response) => {
            // resetForm();
            router.back();
            toast.add({ severity: 'success', summary: `Successo`, detail: 'Mesa criado com sucesso', life: 3000 });
        })
        .catch((error) => {
...
const getData = async (page = 1) => {
    axios
        .get(`/api/paymentmethods/create`, {
            params: {
                query: searchQuery.value
            }
        })
        .then((response) => {
            retriviedData.value = response.data.customer;
