...
const schema = yup.object({
    category_id: yup.string().required().trim().label('Department'),
...
const onSubmit = handleSubmit((values) => {
    ...
    axios
        .post(`/api/reservations/${router.currentRoute.value.params.id}`, values,{
            headers: {
...
const getData = async (page = 1) => {
    axios
        .get(`/api/reservations/${router.currentRoute.value.params.id}/edit`, {
            params: {
                query: searchQuery.value
            }
        })
        .then((response) => {
            retriviedData.value = response.data.subcategory;
...
const deleteData = () => {
    ...
    axios
        .delete(`/api/reservations/${dataIdBeingDeleted.value}`)
        .then(() => {
            ...
            <div class="card flex flex-col gap-4">
                ...
                <form @submit="onSubmit">
                    <div class="flex flex-col gap-2">
                        ...
                        <InputText v-model="name" id="name" placeholder="Nome da SubCategoria" :class="{ 'p-invalid': errors.name }" type="text" />
