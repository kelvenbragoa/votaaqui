...
const getData = async (page = 1) => {
    axios
        .get(`/api/stocktransfers?page=${page}`, {
            params: {
    ...
    <div class="flex flex-col md:flex-row gap-12" v-else>
        <div class="w-full">
            <div class="card flex flex-col gap-4">
                <div class="font-semibold text-xl">Transferência de Stock</div>
                    <DataTable
                    ...
                    <template #header>
                        <div class="flex justify-between">
                            <router-link to="/stock/stocktransfers/create">
                    ...
                    <Column header="REF" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ data.ref }}
                        </template>
                    </Column>
                    <Column header="Centro de Stock de Origem" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ data.stockcenterorigin.name }}
                        </template>
                    </Column>
                    <Column header="Centro de Stock de Destino" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ data.stockcenterdestination.name }}
                        </template>
                    </Column>
                    ...
                    <template #body="{ data }">
                        <!-- <router-link class="m-3" :to="'/stock/stocktransfers/' + data.id + '/edit'"><i class="pi pi-file-edit"></i></router-link>   -->
                        <router-link class="m-3" :to="'/stock/stocktransfers/' + data.id"><i class="pi pi-eye"></i></router-link>
